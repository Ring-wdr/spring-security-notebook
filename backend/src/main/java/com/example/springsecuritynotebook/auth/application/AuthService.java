...
public class AuthService {
    ...
    public TokenPairResponse issueTokens(SubscriberPrincipal principal) {
        ...
        refreshTokenStore.store(principal.getEmail(), tokenPair.refreshToken(), tokenPair.refreshTokenExpiresIn());
        return tokenPair;
    }
    ...
    public TokenPairResponse refresh(String authorizationHeader, RefreshTokenRequest request) {
        ...
        if (refreshTokenStore.shouldReissue(email)) {
            refreshTokenValue = jwtService.generateRefreshToken(email, jwtService.getRefreshTokenExpiresInSeconds());
